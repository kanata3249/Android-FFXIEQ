...
public class VWAtmaSetView extends ListView {
	AtmaInfo mAtmas[];
	FFXIDAO mDao;
	
	public VWAtmaSetView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public boolean bindFFXICharacter(FFXICharacter charinfo) {
		VWAtmaSelectorAdapter adapter;

		mDao = FFXICharacter.getDao();
		mAtmas = new AtmaInfo[VWAtmaSet.ATMA_MAX];
		for (int i = 0; i < mAtmas.length; i++) {
			Atma eq = charinfo.getVWAtma(i);
			if (eq == null) {
		...
		adapter = new VWAtmaSelectorAdapter(getContext(), R.layout.vwatmasetview, mAtmas);
		setAdapter(adapter);
	...
	private class VWAtmaSelectorAdapter extends ArrayAdapter<AtmaInfo> {
		...
		public VWAtmaSelectorAdapter(Context context,
